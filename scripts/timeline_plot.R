...
timeline <- read.csv2(config[["timeline"]]) %>% 
  mutate(start = dmy(start), 
           ...
           rbind(filter(., type %in% c("competences", "community")) %>%
                   arrange(desc(start)),
                 filter(., !(type %in% c("competences", "community"))) %>%
                   arrange(start))
         } %>%
  pivot_longer(col=c(start, end), names_to = "start_end", values_to = "time" ) %>% 
  mutate(name = factor(name, levels = unique(name)),
         type = factor(type, levels=c("competences", "industry", "education", "academia", "community"))) %>%
